...
class QueueNotificationHandler {
  ...
  Future<void> handleQueueUpdate(
      VehicleQueue queue, String? myVehicleId) async {
    ...
    if (myVehicle.isBoarding && _previousQueue != null) {
      final previousVehicle =
          _previousQueue!.vehicles.cast<QueuedVehicle?>().firstWhere(
                (v) => v?.vehicleId == myVehicleId,
                orElse: () => null,
              );
  ...
  Future<void> scheduleNearingDestinationNotification({
    ...
    final scheduledTime =
        arrivalTime.subtract(Duration(minutes: minutesBefore));
  ...
  Future<void> cancelQueueNotifications() async {
    await notificationService
        .cancelNotification(NotificationIds.vehicleApproaching);
    await notificationService.cancelNotification(NotificationIds.timeToBoard);
    await notificationService
        .cancelNotification(NotificationIds.vehicleDeparting);
    await notificationService
        .cancelNotification(NotificationIds.queuePositionChanged);
  }
  ...
  Future<void> cancelTripNotifications() async {
    ...
    await notificationService
        .cancelNotification(NotificationIds.nearingDestination);
  }
...
final queueNotificationHandlerProvider =
    Provider<QueueNotificationHandler>((ref) {
