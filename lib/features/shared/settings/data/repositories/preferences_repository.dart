...
class PreferencesRepositoryImpl implements PreferencesRepository {
  ...
  Future<List<SavedRoute>> getSavedRoutes() async {
    ...
    try {
      ...
      return list
          .map((e) =>
              SavedRouteModel.fromJson(e as Map<String, dynamic>).toEntity())
          .toList();
    } catch (_) {
      return [];
    }
  ...
  Future<void> _saveRoutes(List<SavedRoute> routes) async {
    final prefs = await _getPrefs();
    final models =
        routes.map((r) => SavedRouteModel.fromEntity(r).toJson()).toList();
  ...
  Future<List<SavedSacco>> getSavedSaccos() async {
    ...
    try {
      ...
      return list
          .map((e) =>
              SavedSaccoModel.fromJson(e as Map<String, dynamic>).toEntity())
          .toList();
    } catch (_) {
      return [];
    }
  ...
  Future<void> _saveSaccos(List<SavedSacco> saccos) async {
    final prefs = await _getPrefs();
    final models =
        saccos.map((s) => SavedSaccoModel.fromEntity(s).toJson()).toList();
  ...
  Future<List<SavedPaymentMethod>> getSavedPaymentMethods() async {
    ...
    try {
      ...
      return list
          .map((e) =>
              SavedPaymentMethodModel.fromJson(e as Map<String, dynamic>)
                  .toEntity())
          .toList();
    } catch (_) {
      return [];
  ...
  Future<void> _savePaymentMethods(List<SavedPaymentMethod> methods) async {
    ...
    final models = methods
        .map((m) => SavedPaymentMethodModel.fromEntity(m).toJson())
        .toList();
    await prefs.setString(
        _PreferencesKeys.savedPaymentMethods, jsonEncode(models));
  }
