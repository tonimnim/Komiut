...
final boardingNotifierProvider =
    StateNotifierProvider.autoDispose<BoardingNotifier, AsyncValue<BoardingResult?>>(
  (ref) {
    final datasource = ref.watch(ticketRemoteDataSourceProvider);
...
final filteredTicketsProvider = FutureProvider.autoDispose<List<Ticket>>(
  (ref) async {
    ...
    return tickets.where((ticket) {
      final matchesRoute = ticket.routeInfo.name.toLowerCase().contains(searchQuery) ||
      ...
      final matchesStops = ticket.pickupStop.toLowerCase().contains(searchQuery) ||
